        ...
        if ($User_email != null) {
            ...
            if (count($checkEmployerModel) != 0 && count($checkVerifyModel) == 0) {
                ...
                if ($VerifyModel) {
                    ...
                    $email->setMessage('<a href="' . base_url() . '/repassword/' .$User_email.'/'. $concealotp.'">Click here to Reset Password</a><br><br>');
                    $email->send();
                    $response = [
                        'messages' => 'success'
                    ];
                ...
                if ($VerifyModel) {
                    ...
                    $email->setTo($to);
                    $email->setSubject('การขอเปลี่ยนรหัสผ่าน');
                    $email->setMessage('ลิงค์มีอายุใช้งาน 15 นาที กรุณายืนยันการขอเปลี่ยนรหัสผ่านตามลิงค์<a href="' . base_url() . '/repassword/' .$User_email.'/'. $concealotp.'">Click here to Reset Password</a><br><br>');
                    $email->send();
                    $response = [
                        'messages' => 'success'
                    ];
    ...
    public function Resetlink($id = null)
    {
        ...
        if (count(array($checkemail)) != 0) {
            ...
            if ($timeNowToArray <  $timeEnd) {
                return redirect()->to('https://flashwork.herokuapp.com/repassword/' . $checkemail[0] . '/' . $id);
            } else {
                return redirect()->to('https://flashwork.herokuapp.com/repassword/' .  'unknow' . '/' . 'unknow');
            }
        } else {
            return false;
        };
